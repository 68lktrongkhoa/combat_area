...
function getAiInstance(lang: 'en' | 'vi'): GoogleGenAI {
  if (ai) {
    return ai;
  }
  
  const apiKey = 'AIzaSyDyOuoCHVg_3j92Eqlsw75pTRzUbt0ByLI';
  
  if (!apiKey) {
    const errorMsg = lang === 'vi' 
        ? "Lỗi Cấu hình: API_KEY không được đặt. Vui lòng kiểm tra các biến môi trường của bạn."
        : "Configuration Error: API_KEY is not set. Please check your environment variables.";
